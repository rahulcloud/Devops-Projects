...
1. Create *Jenkins job*, Fill the following details,
   ...
   - *Post Steps*
     ...
     - *Send files or execute commands over SSH*
       ...
       - Exec Command: 
	      - `cd /opt/docker`
          - `docker build -t tek_demo .`
	      - `docker tag tek_demo tek/demo`
          - `docker push tek/tek_demo`
          - `docker rmi tek_demo tek/tek_demo`
              
1. Login to Docker host and check images and containers. (no images and containers)

1. login to docker hub and check. shouldn't find images with for tek_demo 

1. Execute Jenkins job

1. check images in Docker hub. Now you could able to see new images pushed to tek Docker_Hub
     ...
     - hosts: web-servers
       ...
       tasks:
        - name: stop previous version docker
          shell: docker stop tek_demo
        - name: remove stopped container
          shell: docker rm -f tek_demo	  
        - name: remove docker images
          shell: docker image rm -f tek/tek_demo
          
        - name: create docker image
          shell: docker run -d --name tek_demo -p 8090:8080 tek/tek_demo
...
1. Create Jenkins job 
   ...
   - *Send files or execute commands over SSH*
     ...
     - Remote directory	: `//opt//docker`
        ...
        - `docker tag $JOB_NAME:v1.$BUILD_ID tek/$JOB_NAME:v1.$BUILD_ID`
        - `docker tag $JOB_NAME:v1.$BUILD_ID tek/$JOB_NAME:latest`
        - `docker push tek/$JOB_NAME:v1.$BUILD_ID`
        - `docker push tek/$JOB_NAME:latest`
        - `docker rmi $JOB_NAME:v1.$BUILD_ID tek/$JOB_NAME:v1.$BUILD_ID` 
        - `tek/$JOB_NAME:latest`
##### References
